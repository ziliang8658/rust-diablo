    ...
    while running {
            ...
            match event {
                ...
                sdl2::event::Event::KeyUp { keycode: Some(keycode), .. } => {
                    match keycode {
                        ...
                        sdl2::keyboard::Keycode::D | sdl2::keyboard::Keycode::Right => key_right = false,
                        _ => {}
                    }
                }
                _ => {}
        ...
        if direction != last_direction {
            ...
            if direction_changes <= 10 {
                println!("Direction change #{}: {:?} -> Animation: {}", 
                    direction_changes,
                    direction,
                    if direction.is_moving() { "Walk" } else { "Idle" }
                );
            }
        }
