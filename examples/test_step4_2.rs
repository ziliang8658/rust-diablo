use rust_diablo::engine::{Engine, Direction};
use rust_diablo::world::{World, TileType};
use rust_diablo::entity::{Entity, EntityType};
use rust_diablo::renderer::{Color, Camera};
...
fn main() -> Result<()> {
    ...
    let mut event_pump = engine.sdl_context()
        .event_pump()
    ...
    while running {
        ...
        if keys.contains(&sdl2::keyboard::Keycode::Up) { move_y -= 1.0; }
        if keys.contains(&sdl2::keyboard::Keycode::Down) { move_y += 1.0; }
        if keys.contains(&sdl2::keyboard::Keycode::Left) { move_x -= 1.0; }
        if keys.contains(&sdl2::keyboard::Keycode::Right) { move_x += 1.0; }
