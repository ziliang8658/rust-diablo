...
fn main() -> Result<()> {
    ...
    println!("✓ MIN loaded: {} pieces, {} blocks per piece\n", min_data.len(), min_data.blocks_per_piece);
    ...
    for piece_idx in 0..5.min(min_data.len()) {
        ...
        if let Some(piece) = min_data.get(piece_idx) {
            for (block_idx, block) in piece.mt.iter().take(piece.mt.len().min(4)).enumerate() {
                println!("    Block[{}]: data={:#06x}, has_value={}, type={:?}, frame={} (1-based)",
                    block_idx, block.data, block.has_value(), block.tile_type(), block.frame());
            }
            if piece.mt.len() > 4 {
        ...
        for block_idx in 0..4.min(min_data.blocks_per_piece) {
            match texture_mgr.get_decoded_tile(piece_idx, block_idx) {
                Ok(rgba) => {
                    ...
                    let all_black = rgba.chunks(4).filter(|p| p[0] == 0 && p[1] == 0 && p[2] == 0 && p[3] == 255).count();
                    println!("    Block[{}]: {} pixels, {} non-transparent, {} black",
    ...
    println!("  Range: {} to {} (should be 0 to {})", min_val, max_val, min_data.len() - 1);
