...
fn main() {
    ...
    if let Ok(vcpkg_root) = env::var("VCPKG_ROOT") {
        ...
        // 方法2: 检查 installed 目录
        let vcpkg_installed = PathBuf::from(&vcpkg_root).join("installed").join("x64-windows");
        if vcpkg_installed.exists() {
            let lib_dir = vcpkg_installed.join("lib");
            if lib_dir.exists() {
                println!("cargo:rustc-link-search=native={}", lib_dir.display());
                if !found_zlib {
                    found_zlib = lib_dir.join("zlib.lib").exists() || lib_dir.join("zlibd.lib").exists();
                }
                if !found_bzip2 {
    ...
    if !found_zlib || !found_bzip2 {
        println!("cargo:warning=zlib found: {}, bzip2 found: {}", found_zlib, found_bzip2);
