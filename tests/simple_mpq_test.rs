...
use rust_diablo::resources::mpq_format::{hash_string, HashType, decrypt_block, MPQ_SIGNATURE};
...
fn test_simple_mpq_read() {
    ...
    let signature = u32::from_le_bytes([header_bytes[0], header_bytes[1], header_bytes[2], header_bytes[3]]);
    let hash_table_offset = u32::from_le_bytes([header_bytes[16], header_bytes[17], header_bytes[18], header_bytes[19]]);
    let hash_table_entries = u32::from_le_bytes([header_bytes[24], header_bytes[25], header_bytes[26], header_bytes[27]]);
    
    println!("\n=== MPQ Header ===");
    println!("Signature: 0x{:08X} (expected: 0x{:08X})", signature, MPQ_SIGNATURE);
    ...
    // 读取哈希表（加密）
    file.seek(SeekFrom::Start(hash_table_offset as u64)).unwrap();
    ...
    println!("\n=== Before Decryption ===");
    println!("u32[0-3]: 0x{:08X} 0x{:08X} 0x{:08X} 0x{:08X}", 
    ...
    if block_index < hash_table_entries {
        ...
        println!("  ✗ INVALID block index! (should be < {} or 0xFFFFFFFF/0xFFFFFFFE)", hash_table_entries);
    }
}
