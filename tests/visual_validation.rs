...
fn test_export_sample_tiles() {
    ...
    let palette =
        Palette::from_mpq(&mut mpq, "levels/towndata/town.pal").expect("Failed to load palette");
    ...
    for i in 1..min_data.len().min(1000) {
        // Limit to first 1000 for efficiency
...
fn test_export_decoded_tiles_direct() {
    ...
    let cel_data = mpq
        .find_file("levels/l1data/l1.cel")
    ...
    let min_data =
        MinData::load_for_dungeon(&mut mpq, DungeonType::Cathedral).expect("Failed to load MIN");

    let palette =
        Palette::from_mpq(&mut mpq, "levels/towndata/town.pal").expect("Failed to load palette");

    println!(
        "Loaded: {} CEL frames, {} micro tiles",
    ...
    for i in 1..min_data.len().min(2000) {
        ...
        if *count < 5 {
            // Collect 5 samples of each type
            let frame_idx = block.frame() as usize;
            ...
            match decode_tile(tile_type, &raw_data) {
                Ok(decoded) => {
                    ...
                    *count += 1;
                    println!(
                        "  ✓ Collected {:?} sample #{} (frame {})",
                        tile_type, *count, frame_idx
                    );
                }
                Err(e) => {
...
fn test_create_comparison_grid() {
    ...
    let sample_refs: Vec<(&str, &[u8], u32, u32)> = samples
        .iter()
