...
use rust_diablo::tiles::{DungeonType, MinData, TilData};
...
fn test_sector1s_resource_indices() {
    ...
    println!(
        "MIN Data: {} pieces, {} blocks per piece",
        min_data.len(),
    ...
    for j in 0..height {
        for i in 0..width {
            ...
            if let Some(mega_tile) = til_data.get(mega_tile_idx) {
                ...
                println!(
                    "    micro1 (top-left):   {} (levelPieceId, 1-based)",
                    mega_tile.micro1
                );
                println!(
                    "    micro2 (top-right):  {} (levelPieceId, 1-based)",
                    mega_tile.micro2
                );
                println!(
                    "    micro3 (bottom-left): {} (levelPieceId, 1-based)",
                    mega_tile.micro3
                );
                println!(
                    "    micro4 (bottom-right): {} (levelPieceId, 1-based)",
                    mega_tile.micro4
                );
                    ...
                    if let Some(piece) = min_data.get(piece_idx) {
                        ...
                        for (block_idx, block) in piece.mt.iter().enumerate() {
                            if block.has_value() {
                                ...
                                println!(
                                    "      Block[{}]: frame={} (1-based CEL index), type={:?}",
                        ...
                        println!(
                            "    {}: levelPieceId={} -> MIN Piece[{}] NOT FOUND",
            ...
            .map(|j| {
                (0..width)
                    .filter(|i| {
                        ...
                        if idx + 1 < tile_layer.len() {
                            let mega_tile_id =
                                u16::from_le_bytes([tile_layer[idx], tile_layer[idx + 1]]);
                            mega_tile_id != 0
                        } else {
                            false
                        }
