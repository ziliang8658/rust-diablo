...
#[cfg(test)]
mod tests {
    use rust_diablo::resources::{MpqManager, PcxImage, ClxSprite, Palette};
    ...
    fn test_load_pcx_from_mpq() {
        ...
        for path in &pcx_paths {
            if let Ok(pcx) = PcxImage::from_mpq(&mut mpq, path) {
                ...
                assert!(pcx.width > 0);
                assert!(pcx.height > 0);
                assert_eq!(pcx.pixels.len(), (pcx.width as usize) * (pcx.height as usize));
    ...
    fn test_pcx_to_rgba() {
        ...
        let palette = palette_paths.iter()
            ...
            if let Ok(clx) = ClxSprite::from_mpq(&mut mpq, path) {
                ...
                if num_frames > 1 {
                    ...
                    if let Some(first_frame) = clx.get_frame(0) {
                        ...
                        println!("All {} frames have consistent dimensions: {}x{}", 
