...
use rust_diablo::engine::isometric::{
    ...
    mega_to_micro, micro_to_mega,
    in_mega_bounds, in_micro_bounds,
};
...
fn test_load_cathedral_min() {
    ...
    for i in 0..10.min(min_data.len()) {
        if let Some(block) = min_data.get(i) {
            if block.has_value() {
                println!("  Tile {}: type={:?}, frame={}", 
                    i, block.tile_type(), block.frame());
            }
        }
    }
}
...
fn test_load_cathedral_til() {
    ...
    for i in 0..5.min(til_data.len()) {
        if let Some(mega_tile) = til_data.get(i) {
            ...
            let has_data = mega_tile.micro1 != 0 || mega_tile.micro2 != 0 
                || mega_tile.micro3 != 0 || mega_tile.micro4 != 0;
            if has_data {
                // At least one micro tile should be non-zero
                assert!(mega_tile.micro1 > 0 || mega_tile.micro2 > 0 
                    || mega_tile.micro3 > 0 || mega_tile.micro4 > 0);
            }
        }
    }
...
fn test_load_cathedral_sol() {
    ...
    assert!(sol_data.len() > 0, "SOL file should contain tile properties");
...
fn test_mega_micro_conversions() {
    ...
    for (mx, my) in test_positions {
        ...
        assert!(in_micro_bounds(micro_x, micro_y), 
            "MicroTile ({}, {}) out of bounds", micro_x, micro_y);
    }
...
fn test_tile_data_consistency() {
    ...
    let min_data_opt = mpq_manager.find_file("levels/l1data/l1.min")
        .and_then(|data| MinData::from_bytes(&data).ok());
    let til_data_opt = mpq_manager.find_file("levels/l1data/l1.til")
        .and_then(|data| TilData::from_bytes(&data).ok());
    let sol_data_opt = mpq_manager.find_file("levels/l1data/l1.sol")
