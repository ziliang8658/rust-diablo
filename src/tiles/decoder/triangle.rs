...
pub fn decode_left_triangle(raw_data: &[u8]) -> Result<Vec<u8>> {
    ...
    for i in 0..8 {
        ...
        if src > raw_data.len() {
            bail!(
                "LeftTriangle: insufficient data at pair {} (src={})",
                i,
                src
            );
        }
        ...
        if src + width1 > raw_data.len() {
            bail!(
                "LeftTriangle: insufficient data for row {} (need {})",
                i * 2,
                width1
            );
        }
    ...
    for _ in 0..7 {
        ...
        if src > raw_data.len() {
            bail!(
                "LeftTriangle: insufficient data in upper half (src={})",
                src
            );
        }
...
pub fn decode_right_triangle(raw_data: &[u8]) -> Result<Vec<u8>> {
    ...
    for i in 0..8 {
        ...
        if src + width1 > raw_data.len() {
            bail!(
                "RightTriangle: insufficient data for row {} (need {})",
                i * 2,
                width1
            );
        }
        let offset1 = 32 - width1;
        output[dst + offset1..dst + offset1 + width1].copy_from_slice(&raw_data[src..src + width1]);
        ...
        output[dst + offset2..dst + offset2 + width2].copy_from_slice(&raw_data[src..src + width2]);
        src += width2;
        dst += 32;
    }
