...
use crate::tiles::{DungeonType, MinData, TileType};
...
impl TileTextureManager {
    ...
    pub fn load_for_dungeon(
        ...
        let (cel_path, _special_cel_path, pal_path, min_path): (&str, Option<&str>, &str, &str) =
            match dungeon_type {
                DungeonType::Town => (
        ...
        println!(
            "  Loading special CEL, trying {} paths...",
            special_cel_paths.len()
        );
        ...
        let special_cel_sprite = {
            ...
            for special_cel_path in &special_cel_paths {
                ...
                if let Some(special_data) = mpq_manager
                    .find_file(&special_unix)
                    .or_else(|| mpq_manager.find_file(&special_windows))
                {
                    ...
                    match DungeonCelSprite::from_clx_bytes(&special_data) {
                        Ok(sprite) => {
                            println!(
                                "  ✓ Loaded special CEL from {} as CLX: {} frames",
                                special_cel_path,
                                sprite.frames.len()
                            );
                        ...
                        Err(clx_err) => {
                            ...
                            match DungeonCelSprite::from_cel_bytes_with_width(
                                &special_data,
                                SPECIAL_CEL_WIDTH,
                            ) {
                                Ok(sprite) => {
                                ...
                                Err(cel_err) => {
                                    eprintln!(
                                        "    ⚠ Failed to parse {} as CLX: {}",
                                        special_cel_path, clx_err
                                    );
                                    eprintln!(
                                        "    ⚠ Failed to parse {} as CEL RLE: {}",
                                        special_cel_path, cel_err
                                    );
            ...
            if loaded_sprite.is_none() {
                println!(
                    "  ℹ No special CEL loaded (tried {} paths)",
                    special_cel_paths.len()
                );
            }
        ...
        println!(
            "  ✓ Loaded MIN: {} pieces ({} blocks/piece)",
            min_data.len(),
    ...
    /// Get indexed pixels for a tile (for lighting application)
    ///
    /// # Arguments
    ...
    pub fn get_decoded_foliage(&mut self, piece_index: usize, block_index: usize) -> Result<&[u8]> {
        ...
        let foliage_data =
            &raw_data[REENCODED_TRIANGLE_FRAME_SIZE..REENCODED_TRIANGLE_FRAME_SIZE + 512];

        // Decode foliage as TransparentSquare (16×32)
        let decoded =
            decode_tile(TileType::TransparentSquare, foliage_data).with_context(|| {
                format!(
        ...
        if decoded.len() != 512 {
            bail!(
                "Foliage decoded size mismatch: expected 512 bytes, got {}",
                decoded.len()
            );
        }
    ...
    fn get_indexed_tile(&mut self, frame_idx: usize, tile_type: TileType) -> Result<&[u8]> {
        ...
        let (cel_frame, source_name) = if array_idx < main_cel_len {
            ...
            let adjusted_idx = array_idx - main_cel_len;
            (
                &self.special_cel_sprite.as_ref().unwrap().frames[adjusted_idx],
                "special",
            )
        };
    ...
    pub fn get_frame(
        &self,
        frame_idx: usize,
    ) -> Result<&crate::resources::dungeon_cel::DungeonCelFrame> {
        if frame_idx == 0 {
...
mod tests {
    ...
    fn test_load_cathedral_special_cel() {
        ...
        let tex_mgr =
            TileTextureManager::load_for_dungeon(DungeonType::Cathedral, &mut mpq).unwrap();
        ...
        if let Some(ref special) = tex_mgr.special_cel_sprite {
            ...
            assert!(
                frame_result.is_ok(),
                "Should be able to access special CEL frames"
            );

            if let Ok(frame) = frame_result {
                assert!(
                    !frame.raw_data.is_empty(),
                    "Special CEL frame should have data"
                );
            }
        } else {
