...
pub use til::{TilData, MegaTile};
pub use sol::{SolData, MAXTILES};
pub use decoder::{
    decode_tile,
    decode_square,
    decode_left_triangle,
    decode_right_triangle,
    decode_left_trapezoid,
    decode_right_trapezoid,
    decode_transparent_square,
};
...
impl DungeonTileset {
    ...
    pub fn from_cel_frames(frames: Vec<ClxFrame>) -> Self {
        let texture_ids = (0..frames.len())
            .map(|i| format!("tile_{}", i))
            .collect();
    ...
    pub fn get_texture_id(&self, frame_index: u16) -> Option<&str> {
        ...
        unsafe {
            DEBUG_COUNT += 1;
            if DEBUG_COUNT <= 10 {
                println!("  [get_texture_id] frame_index={} -> index={} -> wrapped={}/{}", 
                    frame_index, index, wrapped_index, self.texture_ids.len());
    ...
    pub fn get_frame_size(&self, frame_index: u16) -> Option<(u32, u32)> {
        ...
        self.frames.get(wrapped_index).map(|f| (f.width as u32, f.height as u32))
    }
