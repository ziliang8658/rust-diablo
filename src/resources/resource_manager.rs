...
use crate::resources::{
    Cl2Sprite, ClxFrame, ClxSprite, ColorTransform, MpqManager, Palette, PcxImage,
};
/// 资源管理器 - 统一资源加载和缓存管理
///
...
impl ResourceManager {
    ...
    pub fn load_palette(&mut self, path: &str) -> Result<Arc<Palette>> {
        ...
        self.palette_cache
            .insert(path.to_string(), Arc::clone(&palette_arc));
    ...
    pub fn load_trn(&mut self, path: &str) -> Result<Arc<ColorTransform>> {
        ...
        self.trn_cache
            .insert(path.to_string(), Arc::clone(&trn_arc));
    ...
    pub fn load_cl2(
        ...
        path: &str,
        frame_width: u16,
    ) -> Result<Arc<Vec<crate::resources::ClxFrame>>> {
        // 检查缓存
        ...
        self.cl2_cache
            .insert(path.to_string(), Arc::clone(&frames_arc));
    ...
    pub fn load_pcx(&mut self, path: &str) -> Result<PcxImage> {
        ...
        PcxImage::from_bytes(&data).with_context(|| format!("Failed to parse PCX: {}", path))
    }
    ...
    pub fn load_dungeon_cel(
        &mut self,
        path: &str,
    ) -> Result<Vec<crate::resources::DungeonCelFrame>> {
        let data = self
            .mpq_manager
            .find_file(path)
    ...
    pub fn load_tile_png(&self, path: &str) -> Result<ClxFrame> {
        ...
        let img = image::open(path).with_context(|| format!("Failed to load PNG: {}", path))?;
    ...
    pub fn load_pcx_texture(
        ...
        // 创建纹理
        engine
            .load_texture_from_rgba(texture_id, &rgba_data, pcx.width as u32, pcx.height as u32)
