...
impl Palette {
    /// Create an empty palette (all colors black)
    pub fn new() -> Self {
        Self {
            colors: [Color { r: 0, g: 0, b: 0 }; 256],
        }
    }
    ...
    /// Find the nearest color in palette to target RGB
    ///
    /// Used by lighting system to find closest palette color
...
mod tests {
    ...
    fn test_find_nearest_color() {
        ...
        // Create test palette with specific colors
        data[0] = 0;
        data[1] = 0;
        data[2] = 0; // Index 0: Black (0,0,0)
        data[3] = 255;
        data[4] = 0;
        data[5] = 0; // Index 1: Red (255,0,0)
        data[6] = 0;
        data[7] = 255;
        data[8] = 0; // Index 2: Green (0,255,0)
        data[9] = 0;
        data[10] = 0;
        data[11] = 255; // Index 3: Blue (0,0,255)
        data[12] = 128;
        data[13] = 128;
        data[14] = 128; // Index 4: Gray (128,128,128)

        let palette = Palette::from_bytes(&data).unwrap();

        // Find exact matches
        assert_eq!(palette.find_nearest_color((0, 0, 0)), 0); // Black
        assert_eq!(palette.find_nearest_color((255, 0, 0)), 1); // Red
        assert_eq!(palette.find_nearest_color((0, 255, 0)), 2); // Green
        assert_eq!(palette.find_nearest_color((0, 0, 255)), 3); // Blue

        // Find nearest for in-between color
        let nearest = palette.find_nearest_color((100, 100, 100));
        assert_eq!(nearest, 4); // Should find gray (128,128,128)
    }
    ...
    fn test_palette_transparency_index_zero() {
        ...
        for i in 0..=255u8 {
            ...
            assert_eq!(
                rgba_opaque[3], 255,
                "Index {} should be opaque in opaque mode",
                i
            );
        }
    }
    ...
    fn test_palette_image_conversion() {
        ...
        assert_eq!(
            &rgba[last_pixel_offset..last_pixel_offset + 4],
            &[252, 252, 252, 255]
        ); // Almost white
    }
