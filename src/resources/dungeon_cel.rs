...
impl DungeonCelSprite {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        ...
        if offset_table_size > data.len() {
            bail!(
                "Frame offset table extends beyond file: need {} bytes, have {}",
                offset_table_size,
                data.len()
            );
        }
        ...
        println!(
            "Dungeon CEL: {} frames, first frame offset = {}, last offset = {}",
        ...
        for i in 0..num_frames {
            ...
            if frame_start >= frame_end || frame_end > data.len() {
                bail!(
                    "Invalid frame offset at index {}: start={}, end={}, file_size={}",
                    i,
                    frame_start,
                    frame_end,
                    data.len()
            ...
            if i < 3 {
                println!(
                    "  Parsing frame {}: offset=[{}, {}), size={}",
                    i, frame_start, frame_end, frame_size
                );
            }

            // Parse frame - store raw encoded data
            let frame = Self::parse_frame(i, frame_data, frame_size).with_context(|| {
                format!(
                    "Failed to parse frame {} (offset={}, size={})",
    ...
    pub fn from_cel_bytes_with_width(data: &[u8], width: usize) -> Result<Self> {
        ...
        println!(
            "CEL file: maybe_num_frames={}, file_size={}",
            maybe_num_frames,
            data.len()
        );
        ...
        let is_single_group = if last_offset_pos + 4 <= data.len() {
            ...
            println!(
                "  Checking: offset[{}] at pos {} = {} vs file_size={}",
                maybe_num_frames,
                last_offset_pos,
                last_offset,
                data.len()
            ...
            println!(
                "  Offset table too short: need {} bytes, have {}",
                last_offset_pos + 4,
                data.len()
            );
            false
        ...
        let num_frames = if is_single_group {
            ...
            bail!(
                "Multi-group CEL files not yet supported (first_u32={}, file_size={})",
                maybe_num_frames,
                data.len()
            );
        };
        ...
        for frame_idx in 0..num_frames {
            ...
            if frame_start >= frame_end || frame_end > data.len() {
                bail!(
                    "Invalid frame offset: frame {} start={} end={} (file_size={})",
                    frame_idx,
                    frame_start,
                    frame_end,
                    data.len()
    ...
    fn decode_cel_rle(data: &[u8], width: usize) -> Result<Vec<u8>> {
        ...
        while pos < data.len() {
            ...
            while row_width < width && pos < data.len() {
                ...
                if control >= 0x80 {
                    ...
                    if pos + opaque_count > data.len() {
                        ...
                        bail!(
                            "CEL RLE data truncated: need {} bytes at pos {}, only {} bytes remain",
                            opaque_count,
                            pos,
                            data.len() - pos
                        );
            ...
            if row_width != width {
                println!(
                    "    ⚠️ Row {} width mismatch: got {} expected {} (row_start={})",
    ...
    pub fn from_clx_bytes(data: &[u8]) -> Result<Self> {
        ...
        // Parse as CLX format
        let clx_sprite =
            ClxSprite::from_bytes(data).context("Failed to parse special CEL as CLX format")?;
    ...
    fn test_from_clx_bytes_conversion() {
        ...
        match DungeonCelSprite::from_clx_bytes(&clx_data) {
            ...
            Err(e) => {
                ...
                println!(
                    "Note: CLX parsing failed (expected for minimal test data): {}",
                    e
                );
            }
        }
