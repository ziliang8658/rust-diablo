...
impl MpqArchive {
    ...
    fn find_file_number(&self, filename: &str) -> Option<u32> {
        ...
        for test_filename in &filenames {
            ...
            unsafe {
                let result =
                    libmpq__file_number(self.handle, c_filename.as_ptr(), &mut file_number);
...
mod tests {
    ...
    fn test_load_diabdat_mpq() {
        ...
        match manager.load_mpq("Diabdat.mpq", 1000) {
            ...
            Err(e) => {
                println!(
                    "⚠ DIABDAT.MPQ not found (this is OK if file doesn't exist): {}",
                    e
                );
            }
        }
        ...
        if manager.load_mpq("Diabdat.mpq", 1000).is_ok() {
            ...
            for path in paths {
                if let Some(pal_data) = manager.find_file(path) {
                    println!(
                        "✓ Successfully read town.pal from {}: {} bytes",
                        path,
                        pal_data.len()
                    );
